...
Sprite::Sprite(HWND hWnd, RECT clientRect)
{
	...
	image = (HBITMAP)LoadImage(0, L"knite.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
...
void Sprite::DrawRectangle(HWND hWnd)
{
	...
	SelectObject(memDC, CreateSolidBrush(RGB(255, 0, 20)));
...
void Sprite::MakeAStep(CHAR key)//управление с помощью клавиш WASD
{
	switch (key)
	{
	case 'W':
...
void Sprite::Draw(HWND hWnd)
{
	...
	{
		properties.width = 150.0f;
		properties.height = 100.0f;
		MakeABounce();
		DrawRectangle(hWnd);
	}
}
